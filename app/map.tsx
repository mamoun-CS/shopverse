  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (hasPermission) {
        ...
        Alert.alert('Location Permission', 'Please enable location services to see nearby stores.');
      }
    };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.storesList}>
        ...
        {sortedStores.slice(0, 3).map((store) => (
          <TouchableOpacity
            key={store.id}
            style={[styles.storeListItem, selectedStore?.id === store.id && styles.storeListItemActive]}
            onPress={() => handleMarkerPress(store)}
          >
            <View style={styles.storeListIcon}>
              <Text style={styles.storeListEmoji}>🏪</Text>
            </View>
